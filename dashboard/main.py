...
def fetch_price_data():
    url = "http://127.0.0.1:80/api/prices/symbol"
    try:
...
def fetch_account_data():
    url = "http://127.0.0.1:80/api/accounts/data"
    try:
        response = requests.get(url)
        data = response.json()
        q_account.put(data)
    except Exception as e:
        q_account.put(f"Error fetching account data: {e}")
...
def fetch_position_data():
    url = "http://127.0.0.1:80/api/positions/data"
    try:
        response = requests.get(url)
        data = response.json()
        q_position.put(data)
    except Exception as e:
        q_position.put(f"Error fetching account data: {e}")
...
def fetch_sum_data():
    price_url= "http://127.0.0.1:80/api/prices/symbol"
    position_url = "http://127.0.0.1:80/api/positions/data"
    acc_url = "http://127.0.0.1:80/api/accounts/data"
    ...
    if not q_price_sum.empty() or not q_account_sum.empty() or not q_position_sum.empty():
        ...
        for acc, symbols in account_symbol_info.items():
            ...
            for symbol, info in symbols.items():
                ...
                total_floatpl = info['total_floatpl']
                market_price = info['market_price']
                side = info['side']
                vwap_price = calculate_vwap(info["side"],total_size,info["capital"],info['comm_swap'])
                
                tree2.insert('', 'end', values=[
                acc, symbol,total_size,side,f"{round(total_floatpl,2):,}",market_price,f"{round(vwap_price,5):,}"
            ])
                # print(vwap_price)
...
def create_table_account(parent, cols):
    tree = ttk.Treeview(parent, columns=cols, show="headings")
    for col in cols:
        tree.heading(col, text=col, anchor="center")
        tree.column(col, anchor="center")
...
def create_table_positions(parent, cols):
    tree = ttk.Treeview(parent, columns=cols, show="headings")
    for col in cols:
        tree.heading(col, text=col, anchor="center")
        tree.column(col, anchor="center")
...
def create_table_sum(parent,cols1, cols2):
    # Create the Treeview widget
    tree1 = ttk.Treeview(parent, columns=cols1, show="headings")
    tree2 = ttk.Treeview(parent, columns=cols2, show="headings")
    for col in cols1:
...
def startgui():
    ...
    tree2 = create_table_account(tab2, cols2)
    ...
    tree3 = create_table_positions(tab3,cols3)

    #tab 4: Total 
    tab4 = ttk.Frame(notebook)
    notebook.add(tab4,text="Summary")
    cols4_1 = ("Account", "Balance", "Total Floatpl", "Closepl", "Equity", "Margin", "Margin Level", "NOP")
    cols4_2 = ("Account", "Symbol", "Size","Type", "Float PL","Market price", "Vwap price")
    tree41,tree42 = create_table_sum(tab4,cols4_1,cols4_2)
